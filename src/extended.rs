...
mod tests {
    ...
    fn div_by_255_reference(v: u16) -> u16 {
        (v as f64 / 255.0).round() as u16
    }

    #[test]
    fn div_255_optimization() {
        for v in 0..(255 * 255) + 128 {
            let opt = div_by_255(v);
            let slow = div_by_255_reference(v);
            assert_eq!(opt, slow);
        }
    }

    fn rounding_div_reference(a: u16, b: u8) -> u16 {
        (a as f64 / b as f64).round() as u16
    }

    #[test]
    fn rounding_div_optimization() {
        for a in 0..255 * 255 {
            for b in 1..255 {
                let opt = rounding_div(a, b);
                let slow = rounding_div_reference(a, b);
                if opt != slow {
                    panic!("Mismatch in results! opt: {opt}, slow: {slow}, calculation: {a}/{b}");
                }
            }
        }
    }
